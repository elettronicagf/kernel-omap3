...
static void ccdc_configure(struct isp_ccdc_device *ccdc)
{
	...
	if (pdata->is_bt656)
		isp_reg_writel(isp,
			(format->height/2 - 1) << ISPCCDC_VDINT_0_SHIFT,
	...
	isp_reg_writel(isp, 2 << ISPCCDC_VERT_START_SLV0_SHIFT |
			2 << ISPCCDC_VERT_START_SLV1_SHIFT,
		       OMAP3_ISP_IOMEM_CCDC, ISPCCDC_VERT_START);

	if (pdata->is_bt656)
		isp_reg_writel(isp, ((format->height >> 1) - 2)
	...
	if (pdata->is_bt656) {
		...
		ccdc_config_outlineoffset(ccdc, pix.bytesperline, 0, 0);
	}
	/* CCDC_PAD_SOURCE_VP */
...
static void ccdc_vd0_isr(struct isp_ccdc_device *ccdc)
{
	...
	int restart = 0;
	struct isp_device *isp = to_isp_device(ccdc);
//	printk(KERN_INFO "ispccdc_vd0_isr\n");
	if (ccdc->output & CCDC_OUTPUT_MEMORY) {
