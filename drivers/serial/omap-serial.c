...
#define DIR_485_GPIO	23
#define DIR_485_GPIO_RECEIVE_ENABLE	0
#define DIR_485_GPIO_TRANSMIT_ENABLE	1
#endif
...
static inline void serial_omap_update_rts(struct uart_omap_port *up)
{
	...
	if ((up->rs485.flags & SER_RS485_ENABLED) &&
			((up->tx_in_progress && rts_on_send) ||
			 !(up->tx_in_progress || rts_on_send)))
		gpio_set_value(DIR_485_GPIO,DIR_485_GPIO_TRANSMIT_ENABLE);
	else
		gpio_set_value(DIR_485_GPIO,DIR_485_GPIO_RECEIVE_ENABLE);
...
static void serial_omap_set_mctrl(struct uart_port *port, unsigned int mctrl)
{
	...
	dev_dbg(up->port.dev, "serial_omap_set_mctrl+%d mctrl=%0x up->mcr=%0x\n", up->pdev->id,mctrl,up->mcr);
	if (mctrl & TIOCM_RTS)
		up->mcr |= UART_MCR_RTS;
	else
		up->mcr &= ~UART_MCR_RTS;
...
#ifdef CONFIG_MACH_OMAP3_EGF
		if (cpu_is_omap3630() && (pdev->id == 3)){
			ret = gpio_request(DIR_485_GPIO, "UART4 RS485");
			if (ret < 0) {
				printk(KERN_ERR "Unable to get DIR_485_GPIOGPIO\n");
			}
			gpio_direction_output(DIR_485_GPIO, DIR_485_GPIO_RECEIVE_ENABLE);
		}
#endif
